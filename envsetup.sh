...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${CREEPER_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    CREEPER_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/creeper/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
...
function eat()
{
    ...
    if (adb shell getprop ro.creeper.device | grep -q "$CREEPER_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $CREEPER_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.creeper.device | grep -q "$CREEPER_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/creeper/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
